        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 relative">
          {HOW_IT_WORKS_STEPS.map((step, index) => (
            <motion.div
              ...
              className="relative p-7 rounded-2xl bg-[#FAF8F5] border border-[#E8D8CC] hover:border-[#CFAFA4] hover:shadow-sm transition-all flex flex-col justify-between"
            >
              <div>
                {/* Step Number & Icon */}
                <div className="flex items-center justify-between mb-6">
                  <div className="w-11 h-11 rounded-full bg-white border border-[#CFAFA4] flex items-center justify-center text-[#5B4942] shadow-xs">
                    {getStepIcon(step.stepNumber)}
                  </div>
                  <span className="font-serif text-3xl font-bold text-[#CFAFA4]">
                    {step.stepNumber}
                  </span>
                </div>
              ...
              <div className="pt-5 mt-6 border-t border-[#E8D8CC] flex items-center justify-between text-[10px] uppercase tracking-wider text-[#7A6A63] font-semibold">
                ...
                {index < 3 && <ArrowRight className="w-3.5 h-3.5 text-[#CFAFA4] hidden lg:block" />}
              </div>
            </motion.div>
          ))}
        </div>
